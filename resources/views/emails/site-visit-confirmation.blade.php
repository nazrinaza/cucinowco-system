...
@section('content')
    ...
    <p style="margin:0 0 24px;color:#4c5159;font-size:15px;line-height:1.7">We have received your complimentary site visit request. Our team will contact you for the location and confirm the appointment.</p>
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 26px;background:#f7f4ec;border-left:5px solid #f5b800">
        <tr><td style="padding:20px">
            ...
            <span style="color:#73777d;font-size:13px;line-height:1.7">{{ $siteVisit->space_label }}<br>{{ $siteVisit->clean_types_label }}<br>{{ $siteVisit->preferred_date?->format('d M Y') }} &middot; {{ str($siteVisit->preferred_time_slot)->replace('_', ' ')->title() }}<br>{{ $siteVisit->site_address ?: 'Location to be confirmed with you.' }} {{ $siteVisit->postcode }}</span>
        </td></tr>
    </table>
