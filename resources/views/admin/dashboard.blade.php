<x-layouts.admin title="Overview" heading="Good {{ now()->hour < 12 ? 'morning' : (now()->hour < 18 ? 'afternoon' : 'evening') }}, {{ str(auth()->user()->name)->before(' ') }}.">
    <div class="metric-grid">
        <article><span>Active quotes</span><strong>{{ number_format($metrics['quotes']) }}</strong><a href="{{ route('admin.quotes.index') }}">Review quotes &rarr;</a></article>
        ...
        <article class="metric-dark"><span>Revenue this year</span><strong>RM {{ number_format($metrics['revenue'], 2) }}</strong><small>{{ number_format($metrics['customers']) }} customers &middot; {{ number_format($metrics['subscribers']) }} subscribers</small></article>
    </div>
    <div class="dashboard-grid">
        <section class="admin-card"><div class="card-head"><div><p>Sales pipeline</p><h2>Recent quote requests</h2></div><a href="{{ route('admin.quotes.index') }}">View all</a></div>
            <div class="table-wrap"><table><thead><tr><th>Reference</th><th>Customer</th><th>Status</th><th>Total</th></tr></thead><tbody>@forelse($recentQuotes as $quote)<tr><td><a href="{{ route('admin.quotes.show',$quote) }}">{{ $quote->quote_number }}</a><small>{{ $quote->created_at->diffForHumans() }}</small></td><td>{{ $quote->customer->name }}<small>{{ $quote->customer->phone }}</small></td><td><span class="status status-{{ $quote->status }}">{{ ucfirst($quote->status) }}</span></td><td>RM {{ number_format($quote->total,2) }}</td></tr>@empty<tr><td colspan="4" class="empty-cell">New website requests will appear here.</td></tr>@endforelse</tbody></table></div>
        </section>
